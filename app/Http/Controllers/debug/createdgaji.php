...
use PDF;
class createdgaji extends Controller
{
    // created gaji
    public function create(){
        ...
        $pegawai=pegawai::all();
        $check_create = mastergaji::whereDate('created_at','>=',$tgl_pertama)->whereDate('created_at','<=',$tgl_terakhir)->value('id');
        if(empty($check_create)=== true){
            return response()->json(['success create gaji'], 200);
        }
        elseif(empty($check_create)=== false){
            return response()->json(['data sudah di create'],400);
        }
        ...
        //     $waktu_kerja_all = JadwalShift::whereMonth('date', $bulan_ini)->where('Ids',$id_user)->get()->count();
        //     $waktu_kerja_libur = JadwalShift::where('shift','libur')->where('Ids',$id_user)->get()->count();
...
// cd /home/hrib6497/public_html && php artisan gaji:created >> /dev/null 2>&1